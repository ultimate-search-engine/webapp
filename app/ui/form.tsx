...
function Form() {
  ...
  const submitSearch = async (event: any) => {
    ...
    router.push(
      `/search?q=${event.target.elements.search.value}&length=16&pagination=0`
    );
  };

  const [userQ, setUserQ] = useState<any>(searchParams.get("q") ? searchParams.get("q") : "");
  ...
  const onInputChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      if (event.target.value.length > 0) {
        ...
        let data_fixed = data.autocomplete.concat(
          data.next_words.map((el: String) => event.target.value + " " + el)
        );
