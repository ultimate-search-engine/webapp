...
import {ResultType, SpecialType} from "../scripts/interfaces";
    ...
    return (
        <div>
            <div className={styles.container}>
                <Header title={query.q} searchField={true}/>
                <main className={styles.main}>
                    <div className={styles.grid_container}>
                        <div className={styles.results} id={'results'}>
                            {results.length ? results.map((element: ResultType, i: number) => {
                                return (<Result data={element} key={i}/>)
                            }) : <Noresult/>}
                        </div>
                        <div className={styles.special}>
                            {/*<Calculator/>*/}
                            {special ? special.wiki.map((element: SpecialType, i: number) => {
                                return (<Special_wiki data={element} key={i}/>)
                            }) : <p></p>}
                            {special ? special.goodreads.map((element: SpecialType, i: number) => {
                                return (<Special_goodreads data={element} key={i}/>)
                            }) : <p></p>}
                        </div>
                    </div>
                </main>
                <Footer data={typeof (query.pagination) == 'string' ? parseInt(query.pagination) : 1}
        ...
        for (let i = 0; i < data.fulltext.results.length; i++) {
            if (data.fulltext.results[i].type == "default") {
                ...
                if (tempResults[tempResults.length - 1].title.length > 60) {
                    let temp = tempResults[tempResults.length - 1].title.substring(0, 60).split(' ')
                    temp.splice(-1)
                ...
                if (!tempSpecial.goodreads.length && data.fulltext.results[i].url.split('/')[3] === 'book') {
                    // @ts-ignore
